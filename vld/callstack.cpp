...
int CallStack::resolve(BOOL showInternalFrames)
{
    ...
    for (UINT32 frame = 0; frame < m_size; frame++)
    {
        ...
        if (!foundline)
        {
            // without a line number, it's meaningless
...
bool CallStack::isInternalModule(const PWSTR filename) const
{
    ...
    bool ret =
        ...
        endWith(filename, len, L"\\include\\xmemory") ||
        endWith(filename, len, L"\\include\\memory") ||
        endWith(filename, len, L"\\include\\list") ||
        endWith(filename, len, L"\\include\\xhash") ||
        endWith(filename, len, L"\\include\\vector") ||
        endWith(filename, len, L"\\include\\xstring") ||
        endWith(filename, len, L"\\include\\utility") ||
        endWith(filename, len, L"\\include\\thread") ||
        endWith(filename, len, L"\\include\\xtree") ||
        endWith(filename, len, L"\\include\\map") ||
        endWith(filename, len, L"\\include\\stack") ||
        endWith(filename, len, L"\\include\\deque") ||
        endWith(filename, len, L"\\include\\queue") ||
        endWith(filename, len, L"\\include\\unordered_map") ||
        endWith(filename, len, L"\\include\\xutility") ||
        endWith(filename, len, L"\\include\\type_traits") ||
        endWith(filename, len, L"\\include\\functional") ||
        endWith(filename, len, L"\\include\\algorithm") ||
        endWith(filename, len, L"\\include\\set") ||
        endWith(filename, len, L"\\include\\condition_variable") ||
        // default
        (false);
    return ret;
}

    ...
    if (m_status & VLD_STATUS_INSTALLED) {
        ...
        if (m_status & VLD_STATUS_NEVER_ENABLED) {
            ...
            if (leaks_count == 0) {
                ...
                double totalKb = m_curAlloc / 1024.0;
                double totalMb = m_curAlloc / (1024.0 * 1024.0);
                Report((leaks_count > 1) ? L"s (%Iu bytes / %.3f kb / %.5f mb).\n" : L" (%Iu bytes / %.3f kb / %.5f mb).\n", m_curAlloc, totalKb, totalMb);
...
VOID VisualLeakDetector::configure()
{
    ...
    if (LoadBoolOption(L"ReportFilter", L"off", inipath))
    {
        m_options |= VLD_OPT_REPORT_FILTER;
    }
    ...
    m_reportFilterLessBytes = LoadIntOption(L"ReportFilterLessBytes", VLD_DEFAULT_REPORT_FILTER_LESS_BYTES, inipath);
    if (m_reportFilterLessBytes < 1)
    {
        m_reportFilterLessBytes = VLD_DEFAULT_REPORT_FILTER_LESS_BYTES;
    }

    // ReportMaxLeakSizeRankingsCount
    m_reportMaxLeakSizeRankingsCount = LoadIntOption(L"ReportMaxLeakSizeRankingsCount", VLD_DEFAULT_REPORT_MAX_LEAK_SIZE_RANKINGS_COUNT, inipath);
...
VOID VisualLeakDetector::reportConfig()
{
    ...
    if (m_options & VLD_OPT_REPORT_FILTER)
    {
        Report(L"    Report Filter ON: %u bytes\n", m_reportFilterLessBytes);
    }
    Report(L"    ReportMaxLeakSizeRankingsCount: %u\n", m_reportMaxLeakSizeRankingsCount);
    Report(L"\n");
}
...
SIZE_T VisualLeakDetector::reportHeapLeaks(HANDLE heap)
{
    ...
    bool firstLeak = true;
    Set<std::pair<DWORD, SIZE_T>> rankLists;
    SIZE_T leaks_count = reportLeaks(heapinfo, firstLeak,rankLists, aggregatedLeaks);
...
SIZE_T VisualLeakDetector::reportLeaks(heapinfo_t * heapinfo, bool& firstLeak, Set<std::pair<DWORD, SIZE_T>>& rankLists, Set<blockinfo_t*> &aggregatedLeaks, DWORD threadId)
{
    BlockMap* blockmap = &heapinfo->blockMap;
    SIZE_T leaksFound = 0;
    ...
    for (BlockMap::Iterator blockit = blockmap->begin(); blockit != blockmap->end(); ++blockit)
    {
        ...
        SIZE_T totalBytes = size * blockLeaksCount;
        double totalKB = totalBytes / 1024.0;
        double totalMb = totalBytes / (1024.0 * 1024.0);
        // rank
        rankLists.insert(std::make_pair(callstackCRC, totalBytes));
        // do filter
        bool report = true;
        if (m_options & VLD_OPT_REPORT_FILTER)
        {
            report = totalBytes >= m_reportFilterLessBytes;
        }
        if (report)
        {
...
#endif
            Report(L"  Leak Hash: 0x%08X, Count: %Iu, Total %Iu bytes / %.3f kb / %.5f mb\n", callstackCRC, blockLeaksCount, totalBytes, totalKB, totalMb);
            // Dump the call stack.
...
SIZE_T VisualLeakDetector::ReportLeaks()
{
    ...
    for (HeapMap::Iterator heapit = m_heapMap->begin(); heapit != m_heapMap->end(); ++heapit) {
        ...
        UNREFERENCED_PARAMETER(heap);
        heapinfo_t* heapinfo = (*heapit).second;
        leaksCount += reportLeaks(heapinfo, firstLeak,rankLists, aggregatedLeaks);
    }
    // deal rank 
    struct  RankItem {
        DWORD hash;
        ...
        RankItem(DWORD hash, SIZE_T size) {
            this->hash = hash;
            this->size = size;
        }
    };
  ...
  class RankResult
    {
    public:
        RankResult(int count) {
            this->totalCount = count;
            int size = sizeof(RankItem) * count;
            this->items = (RankItem*)malloc( size);
            memset(this->items, 0, size);
        }
        ~RankResult() {
            free(this->items);
        }
        void addItem(DWORD hash, SIZE_T size) {
            int idx = 0;
            for (int i = 0;i < this->totalCount;i++)
            {
                auto& item = this->items[i];
                int minSize = this->items[idx].size;
                if (item.size < minSize)
                {
                    idx = i;
                }
            }
            if (size >= this->items[idx].size)
            {
                this->items[idx].hash = hash;
                this->items[idx].size = size;
                this->addCount++;
            }
        }
        void sort() {
            int size = this->totalCount;
            for (int i = 0; i < size - 1; ++i) {
                for (int j = 0; j < size - i - 1; ++j) {
                    if (this->items[j].size < this->items[j + 1].size) {
                        std::swap(this->items[j], this->items[j + 1]);
                    }
                }
            }
        }
        void report(UINT32 minSize) {
            this->sort();
            bool firstMin = false;
            int min = this->totalCount < this->addCount ? this->totalCount : this->addCount;
            for (int i = 0;i < min;i++)
            {
                auto& item = this->items[i];

                SIZE_T totalBytes = item.size;
                if (totalBytes < minSize && !firstMin)
                {
                    Report(L"------------------------------------------------------------------------------\n");
                    firstMin = true;
                }
                double totalKB = totalBytes / 1024.0;
                double totalMb = totalBytes / (1024.0 * 1024.0);
                Report(L"Rank%2d : Leak Hash 0x%08X, Total %6Iu bytes / %.3f kb / %.5f mb\n", i + 1, item.hash, totalBytes, totalKB, totalMb);
            }
            Report(L"\n");
        }
    private:
    ...
    RankResult rankResult(m_reportMaxLeakSizeRankingsCount);
    for (auto it = rankLists.begin();it != rankLists.end();it++)
    {
        DWORD hash = (*it).first;
        SIZE_T size = (*it).second;
        //Report(L"hash:0x%08X size:%Iu\n", hash, size);
        rankResult.addItem(hash, size);
    }
    rankResult.report(m_reportFilterLessBytes);
    return leaksCount;
}
...
SIZE_T VisualLeakDetector::ReportThreadLeaks(DWORD threadId)
{
    ...
    for (HeapMap::Iterator heapit = m_heapMap->begin(); heapit != m_heapMap->end(); ++heapit) {
        ...
        UNREFERENCED_PARAMETER(heap);
        heapinfo_t* heapinfo = (*heapit).second;
        leaksCount += reportLeaks(heapinfo, firstLeak,rankLists, aggregatedLeaks, threadId);
    }
    return leaksCount;
}
